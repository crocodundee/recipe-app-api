...
class PrivateTagsApiTests(TestCase):
    """Test the login required tags API"""
    ...
    def test_tags_limited_to_user(self):
        """Test tags retrieve only for logged in user"""
        no_log_user = get_user_model().objects.create_user(
            email='no_log_user@company.com',
            password='notestpass'
        )
        tag = Tag.objects.create(user=self.user, name='Fruity')
        Tag.objects.create(user=no_log_user, name='Meat')
    ...
    def test_retrieve_tags_assigned_to_recipes(self):
        """Test filtering tags assigned to recipes only"""
